...
import shaded.org.apache.commons.collections4.iterators.AbstractIteratorDecorator;
import shaded.org.apache.commons.collections4.keyvalue.AbstractMapEntryDecorator;
import shaded.org.apache.commons.collections4.set.AbstractSetDecorator;
...
abstract class AbstractInputCheckedMapDecorator<K, V>
    ...
    private class EntrySet extends AbstractSetDecorator<Entry<K, V>> {
        ...
        protected EntrySet(final Set<Entry<K, V>> set, final AbstractInputCheckedMapDecorator<K, V> parent) {
            super(set);
            this.parent = parent;
        }
        ...
        public Object[] toArray() {
            ...
            for (int i = 0; i < array.length; i++) {
                array[i] = new MapEntry((Entry<K, V>) array[i], parent);
            }
            return array;
        }
        ...
        public <T> T[] toArray(final T[] array) {
            ...
            for (int i = 0; i < result.length; i++) {
                result[i] = new MapEntry((Entry<K, V>) result[i], parent);
            }
    ...
    private class EntrySetIterator extends AbstractIteratorDecorator<Entry<K, V>> {
        ...
        protected EntrySetIterator(final Iterator<Entry<K, V>> iterator,
        ...
        public Entry<K, V> next() {
            final Entry<K, V> entry = getIterator().next();
    ...
    private class MapEntry extends AbstractMapEntryDecorator<K, V> {
        ...
        protected MapEntry(final Entry<K, V> entry, final AbstractInputCheckedMapDecorator<K, V> parent) {
            super(entry);
            this.parent = parent;
        }
